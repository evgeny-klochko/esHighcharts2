var CHARTS = (function (chart) {
  ...
  chart.renderGistogramDividerCompare = function (chartInfo, chartInfo2, configInfo) {
    ...
    hideFirstLabel();
    centerFirstLabel(data);
  }
  return chart;

  function hideFirstLabel() {
    var $firstLabel = $('.highcharts-series-2').find('.highcharts-data-label').first();
    $firstLabel.hide();
  }
  function centerFirstLabel(data) {
    var $firstLabel = $('.highcharts-xaxis-labels').find('span').first();
    var left = data[0].barX + 6;
      ...
      chart: {
        ...
        events: {
          ...
          load: function(event) {
              ...
              $('.highcharts-legend-item rect').attr('width', '50').attr('x', '-20');
          }
        },
        marginLeft: 10,
        marginRight: 10,
